...
struct WholeSch {
  ...
  operator bool() const { return tree; }
  WholeSch copy() const {
    if (!tree)
...
void halv_bat(LTreeNode *node) {
  if (!node->children.empty() &&
      node->children.front()->num_batch == node->num_batch) {
...
public:
  SAEngine(std::uint32_t seed, bool directCout = false)
      : generator(seed), out(directCout ? cout : strStream) {
  ...
  LTreeNode *sa_change(LTreeNode *root, bool *valid_op, lid_t max_depth = 0,
                       int sa_type = 0, int *op_type = nullptr) {
    root = root->copy();
    ...
    do {
      ...
      case 0: {
        ...
        if (network->getNode(l).getPrevs().contains(x))
          break;
      ...
      case 1: {
        ...
        lid_t x = c->layer_set.first();
        if (network->getNode(x).getPrevs().contains(l))
          break;
      ...
      case 2: {
        ...
        grandma->children.insert(grandma->children.begin() + i,
                                 par->children.begin(), par->children.end());
        grandma->stage.clear();
        ...
        if (grandma == root) {
          ...
          while (grandma->parent->parent)
            grandma = grandma->parent;
      ...
      case 3: {
        ...
        LTreeNode *par = lnode->parent;
        if (par == nullptr || par->children.size() <= 2)
          break;
        ...
        new_par = new LTreeNode(Bitset(), lnode->num_batch, nullptr,
                                T_under_T ? LTreeNode::NodeType::T
                                          : LTreeNode::NodeType::L);
        new_par->children.insert(new_par->children.begin(),
                                 par->children.begin() + i,
                                 par->children.begin() + j);
        new_par->parent = par;
        ...
        for (auto x : new_par->children) {
          ...
          if (T_under_T)
            x->t = LTreeNode::NodeType::L;
          else if (par->t == LTreeNode::NodeType::T)
            flat_bat(x);
        }
        // Now we'll reset the whole seg.
        while (new_par->parent->parent)
          new_par = new_par->parent;
      ...
      case 6: {
        ...
        if (put_before ? (node_pos > 0)
          ...
          grandma->stage.clear();
          par->children.erase(par->children.begin() + node_pos);
          grandma->children.insert(grandma->children.begin() + insert_pos,
                                   lnode);
  ...
  void ping_func(volatile bool& stop){
          while(true){
                  ...
                  cout << "[Ping] " << cur_round << ' ' << cur_tries << ' ' <<
  num_tries << endl; l.unlock();
          }
  }
  */

  void SA_search(WholeSch &w_sch, const Cluster &c, lid_t max_depth = 0,
                 int sa_type = 0) {
    ...
    for (int i = 0; i < NUM_OP; ++i)
      valid_op[i] = true;
    if (network->is_chain())
      valid_op[0] = valid_op[1] = false;
    if (cur_node->get_tot_batch() == 1)
      valid_op[4] = valid_op[5] = false;
    num_tries = 0;
    cur_tries = 0;
    ...
    for (; cur_round < nrounds; ++cur_round) {
      if ((cur_round + 1) % print_intv == 0) {
        // unique_lock<mutex> l(m);
        out << cur_round << ' ' << cur_res->get_cost().cost() << ' '
            << (num_tries * 1.0) / print_intv << endl;
        // l.unlock();
        num_tries = 0;
      }
      ...
      LTreeNode *new_tree =
          sa_change(cur_node, valid_op, max_depth, sa_type, &op_type);
      SchNode *new_res;
      if (new_tree->isNew()) {
    ...
    out << "Accept: " << naccept << " (" << (naccept * 100.0) / nrounds << "%)"
        << endl;
    out << "Per OP: ";
    for (int i = 0; i < NUM_OP; ++i) {
      if (i > 0)
...
void LP_search(lid_t num_layer, len_t tot_batch, Cluster &c, WholeSch &w_sch,
  ...
  for (int i = 0; i < num_layer; ++i) {
    ...
    for (int j = -1; j < i; ++j) {
      if (j != -1 && LP_DP[j] == nullptr)
        continue;
      for (len_t l_bat = 1; l_bat <= 4 && l_bat <= tot_batch; l_bat *= 2) {
        for (int last_T = has_S ? 0 : 1; last_T < (has_T ? 2 : 1); ++last_T) {
          if (j == i - 1 && has_S && has_T && last_T > 0)
            break;
          if (j == -1) {
            // Create top T-cut.
            root_Node = new LTreeNode(Bitset(), tot_batch, nullptr,
                                      LTreeNode::NodeType::T);
          } else {
          ...
          if (j == i - 1) {
            ...
            LTreeNode *cur_Cut =
                new LTreeNode(Bitset(), tot_batch, root_Node,
                              (last_T == 0) ? LTreeNode::NodeType::S
                                            : LTreeNode::NodeType::T);
            for (int k = j + 1; k <= i; ++k) {
...
int main(int argc, char **argv) {
  ...
  int mm, nn, xx, yy, ss, bb, rr, ff, xcut, ycut, _NoP_bw, _DRAM_bw, _NoC_bw,
      _mac_dim, _ul3, total_tops;
  string tech, package_type, IO_type, DDR_type;
  cin >> tech >> mm >> nn >> xx >> yy >> ss >> bb >> rr >> ff >> xcut >> ycut >>
      package_type >> IO_type >> _NoP_bw >> DDR_type >> _DRAM_bw >> _NoC_bw >>
      _mac_dim >> _ul3 >> total_tops;
  // assert(total_tops == 2*xx*yy*_mac_dim*_mac_dim);
  // mm means core_microarch: 0 means PolarCore (NVDLA-STYLE), which is
  // recommenmanded; 1 means Eyeriss Core. nn means network choice xx & yy means
  // the number of cores in the x and y axis, respectively. Default noc topology
  // is mesh. ss means stride, whose definition can be found in ISCA2023 SET,
  // HPCA2024GEMINI, ASPLOS2019TANGRAM. bb means batch size (2^n is
  // recommanded). rr means exploration rounds number, which will be multiplied
  // with a coefficient. ff means optimization goal, concrete formula can be
  // found in this main.cpp. xcut & ycut means the chiplet partition granularity
  // at the x and y dimension. the remaining few parameters represent the
  // architecture parameters as their names.
  /********************* INPUT *********************/
  bw_t NoP_bw = _NoP_bw;
  ...
  double IO_die_num = 0;
  int serdes_number = 0; // calculate the number of serdes for all noc nodes
                         // (assuming each side of a node has a serdes)
  if (compute_die_num == 1) {
    ...
    IO_die_num = 4;
    if (IO_type == "XSR") {
      serdes_number = Cluster::ylen * 2 + 2 * Cluster::ylen * NoC::x_cut +
                      2 * Cluster::xlen * NoC::y_cut;
    }
  }
  ...
  //*********************DIE AREA*********************
  double sram_area_per_core =
      (ul3_ + (al1.Size + wl1.Size + ol1.Size) * 16) * SRAM_den;
  double mac_area_per_core =
      16 * vector_len * lane_len * MAC_den + LR_mac_num * LR_mac_den;
  double NoC_area_per_core = NoC::NoC_bw * NoC_den;
  double core_area =
      (sram_area_per_core + mac_area_per_core + NoC_area_per_core) *
      post_layout_scale;
  double core_len = sqrt(core_area);
  // double NoP_len_per_core = NoC::NoP_bw / 4 * NoP_len;
  double PCIe_area = 3000000 * DRAM_bw_each * 4 / 128;
  double IO_die_area =
      DRAM_bw_each * 4 * (DDR_PHY_den + DDR_ctrl_den) + PCIe_area +
      (compute_die_num == 1
           ? 0
           : (NoP_den * NoC::NoP_bw * Cluster::ylen * 2)); // neglect other IOs
  ...
  if (compute_die_num == 1) {
    ...
    D2D_area_per_compute_die =
        2 * NoC::x_step * NoP_den * NoP_bw + 2 * NoC::y_step * NoP_den * NoP_bw;
  }
  compute_die_area = core_len_real * NoC::x_step * core_len_real * NoC::y_step +
                     D2D_area_per_compute_die;
  ...
  if (compute_die_num != 1) {
    ...
    // cost_compute = compute_die_area * compute_die_num / yield_compute_die /
    // 1000000 * cost_silicon_mm_compute/wafer_util(compute_die_area); cost_IO =
    // IO_die_area / 1000000 * cost_silicon_mm_IO/
    // wafer_util(IO_die_area/4)/yield_IO_die + DRAM_bw_each * 4 / 44.0 *
    // ddr_cost;
    std::vector<ChipConfig> configs = {
        {tech, "Chip", package_type, compute_die_area,
         static_cast<int>(compute_die_num)},
        {tech, "IO", package_type, IO_die_area / 4, 4},
        //{tech, "HBM", package_type, HBM_area, HBM_num}
    };
    auto [total_cost, total_cost_chip, total_cost_package] =
        calculate_cost(configs);
    ...
    // cost_soc = total_die_area / yield_soc / 1000000 *
    // cost_silicon_mm_compute/ wafer_util(total_die_area) + DRAM_bw_each * 4 /
    // 44 * ddr_cost; cost_os_overall = os_area * os_ocst_factor / 1000000 *
    // cost_os;
    std::vector<ChipConfig> configs = {
  ...
  al2.Size = 0;
  al1.RCost = (buffer(vector_len * 8, al1.Size)[0] + 0.1 * lane_len / 8) * 8 *
              turnover_factor * power_factor;
  al1.WCost =
      buffer(vector_len * 8, al1.Size)[1] * 8 * turnover_factor * power_factor;
  wl1.RCost =
      buffer(vector_len * 8, wl1.Size)[0] * 8 * turnover_factor * power_factor;
  wl1.WCost =
      buffer(vector_len * 8, wl1.Size)[1] * 8 * turnover_factor * power_factor;
  ol1.RCost =
      buffer(lane_len * 16, ol1.Size)[0] * 8 * turnover_factor * power_factor;
  ol1.WCost =
      buffer(lane_len * 16, ol1.Size)[1] * 8 * turnover_factor * power_factor;
  ol2.RCost = 0.07648125 * 8 * ul3_ / (1024 KB) * turnover_factor *
              power_factor; // ol2 is a small part of ul3
  ...
  PolarCore core(s, bus, al1, wl1, ol1, al2, wl2, ol2, ul3, LR_mac_num,
                 LR_mac_cost);
  ...
  _al1.RCost =
      0.0509 * 8 * turnover_factor * power_factor; // 8bit IO single port
  _al1.WCost = 0.0506 * 8 * turnover_factor * power_factor; // 0.045;
  _wl1.RCost =
      0.0545 * 8 * turnover_factor * power_factor; // Using 2 banks of 64
  ...
  EyerissCore core2(s2, ibus, wbus, pbus, _al1, _wl1, pl1, ul2, LR_mac_num,
                    LR_mac_cost);
  ...
  for (mlen_t y = 0; y < Cluster::ylen; ++y) {
    if (y < NoC::DRAM_router_num) {
      ...
      NoC::dram_list[3][y - Cluster::ylen / 2] = {
          static_cast<mlen_t>(Cluster::xlen), y};
    }
  }
  ...
  for (mlen_t y = 0; y < Cluster::ylen; ++y) {
    ...
    NoC::dram_list_base[Cluster::ylen + y] = {
        static_cast<mlen_t>(Cluster::xlen), y};
  }
  ...
  /*
          `stschedule` tool prints input parameters, search time, tree info and
     cost info. These output info are directly writed to the `stdout`. Redirect
     `stdout` if needed.
   */
...
#if FORMAT_OUTPUT
  printf("INPUT_PARAMETERS:%s %1d %2d %3d %3d %3d %3d %3d %2d %3d %3d %s %s "
         "%3d %s %8d %3d %4d %4d %8d\n",
         tech.c_str(), mm, nn, xx, yy, ss, bb, rr, ff, xcut, ycut,
         package_type.c_str(), IO_type.c_str(), _NoP_bw, DDR_type.c_str(),
         _DRAM_bw, _NoC_bw, _mac_dim, _ul3, total_tops);
#else
  cout << "INPUT_PARAMETERS: " << tech << ' ' << mm << ' ' << nn << ' ' << xx
       << ' ' << yy << ' ' << ss << ' ' << bb << ' ' << rr << ' ' << ff << ' '
       << xcut << ' ' << ycut << ' ' << package_type << ' ' << IO_type << ' '
       << _NoP_bw << ' ' << DDR_type << ' ' << _DRAM_bw << ' ' << _NoC_bw << ' '
       << _mac_dim << ' ' << _ul3 << ' ' << total_tops << endl;
#endif
  ...
  auto output = [&](int search_index) {
    fprintf(file,
            "%s %1d %2d %3d %3d %3d %3d %3d %2d %3d %3d %s %s %3d %s %8d %3d "
            "%4d %4d %8d %18.5f %20.5f %10lu %28.5f %32.5f %1d %20.2f %20.2f "
            "%20.2f %20.2f %20.2f %20.2f %20.2f %10.3f %10.3f %10.3f %10.3f "
            "%10.3f %10.3f %10.3f\n",
            tech.c_str(), mm, nn, xx, yy, ss, bb, rr, ff, xcut, ycut,
            package_type.c_str(), IO_type.c_str(), _NoP_bw, DDR_type.c_str(),
            _DRAM_bw, _NoC_bw, _mac_dim, _ul3, total_tops, cost_overall,
            scheme->schtree_spm->get_cost().energy,
            scheme->schtree_spm->get_cost().time,
            scheme->schtree_spm->get_cost().cost(),
            scheme->schtree_spm->get_cost().cost() * cost_overall, search_index,
            // Energy breakdown
            SchNode::record.ubuf, SchNode::record.buf, SchNode::record.bus,
            SchNode::record.mac, SchNode::record.NoC_hop_cost,
            SchNode::record.NoP_hop_cost, SchNode::record.DRAM_cost,
            // Cost breakdown
            compute_die_area, IO_die_area, total_die_area, cost_chip,
            cost_package, cost_system_package, cost_soc);
  };
#else
  ofstream writer;
  if (argc > 1) {
  ...
  auto output = [&](int search_index) {
    writer << tech << mm << ' ' << nn << ' ' << xx << ' ' << yy << ' ' << ss
           << ' ' << bb << ' ' << rr << ' ' << ff << ' ' << xcut << ' ' << ycut
           << ' ' << package_type << ' ' << IO_type << ' ' << _NoP_bw << ' '
