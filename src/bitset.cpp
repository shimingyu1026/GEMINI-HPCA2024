...
Bitset::Bitset(Bitset::bitlen_t bit) : std_bs() { set(bit); }
...
Bitset::bitlen_t Bitset::first() const {
  ...
  for (bitlen_t i = 0; i < MAX_BITS_IN_BS; ++i) {
    if (test(i)) {
      return i;
    }
  }
  return MAX_BITS_IN_BS; // No bit set
}

Bitset::bitlen_t Bitset::next(Bitset::bitlen_t bit) const {
  // Find the next set bit after 'bit'
  for (bitlen_t i = bit + 1; i < MAX_BITS_IN_BS; ++i) {
    if (test(i)) {
      return i;
    }
  }
  return MAX_BITS_IN_BS; // No more bits set
}
...
void Bitset::flip(Bitset::bitlen_t bit) { std_bs::flip(bit); }

bool Bitset::contains(Bitset::bitlen_t bit) const { return test(bit); }

void Bitset::set() { std_bs::set(); }

void Bitset::set(Bitset::bitlen_t bit) { std_bs::set(bit); }

void Bitset::reset(Bitset::bitlen_t bit) { std_bs::reset(bit); }

void Bitset::clear() { std_bs::reset(); }
...
Bitset operator|(const Bitset &lhs, const Bitset &rhs) {
  return static_cast<const Bitset::std_bs &>(lhs) |
         static_cast<const Bitset::std_bs &>(rhs);
}
